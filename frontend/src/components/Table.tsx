...
<table >
    ...
    <tbody>
        {logins.map((login) => (
        <tr>
            <td><a href="#" role="button" className="icon-button" onClick={() => onClick(login.id)}><img  src={trash} alt="trash" /></a> {login?.website || "none"}</td>
            <td>{login?.username || "none"}</td>
            <td className="password-column"> {login.password ? (
                  <div>
                    {login.password}
